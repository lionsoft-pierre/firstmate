...
A GitHub Actions check (`Require no-mistakes`) runs on PRs targeting `main` and requires both the deterministic signature and a parseable structured attestation from no-mistakes v1.68.0 or newer.
...
v1.46.0 introduced the attestation, but only v1.68.0 and newer re-attest before every push the pipeline makes, including its own CI-repair pushes; an older build raises PRs whose attestation goes stale on the first repair push and can never pass the gate.
...
3. Initialize the gate with your fork as the push target: `no-mistakes init --fork-url git@github.com:<you>/firstmate.git` (contributing to firstmate requires **no-mistakes v1.68.0+** for head-bound structured attestation; without a fork, plain `no-mistakes init` still works for maintainers with push access).
