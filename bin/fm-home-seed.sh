...
acquire_treehouse_home() {
  ...
  # Lease the home from the ACQUIRING home's own pool root (fm_treehouse_root
  # owns why a home must not share one), so a new secondmate home is never a
  # slot another home's checkout grew.
  ...
  home=$(cd "$FM_ROOT" && treehouse get --lease --lease-holder "$id" --root "$root") || {
