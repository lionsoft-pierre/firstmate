...
5. **Fleet-state digest** - after that read-once contract and ahead of the context digest, the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled once per section as wake-EVENT history, not current state, with each full log path printed for a deeper read); the away posture (`state/.afk-contract`, plus the `state/.afk` daemon flag where a daemon runs); and one cheap alive/dead read of each task's recorded backend endpoint.
...
These skills are not captain-invocable; load each only at its trigger, which its own description states in full.

| Skill | Load |
| --- | --- |
| `bootstrap-diagnostics` | on any actionable digest diagnostic line (the skill names every prefix), or a `BOOTSTRAP_INFO:` interrupted-cleanup note; silence and other `BOOTSTRAP_INFO:` facts need no load |
...
| `firstmate-orca` | before any Orca backend switch, spawn, supervision, smoke test, debugging, or metadata reconciliation |
| `project-management` | before adding, creating, cloning, registering, removing, or initializing a project |
| `stuck-crewmate-recovery` | on a dead or windowless direct report at startup, a stale wake, a looping, confused, unresponsive, or unsteerable worker, or a worker reporting its no-mistakes pipeline dead |
| `secondmate-provisioning` | before creating, seeding, validating, launching, handing backlog to, recovering, pushing inherited material into, or retiring a secondmate home, or editing `data/secondmates.md` |
| `captain-hold-lifecycle` | before treating an investigation or visual review as complete, when recording or routing the captain's answer, and on any `RECORD DIVERGENCE` drain line |
| `process-event-sources` | before arming a long-polling source or a condition->action watch, and on any `procevent`, `process-event source stranded`, or `process-event source failed to start` check wake; never run a registered source's blocking command yourself in a conversational turn |
| `fmx-respond` | on `x-mention`, `x-mode-error`, or `public-followup` check wakes, on any Relay-linked milestone or terminal wake, and on a startup-surfaced public commitment; relevant only when Relay is on |
| `firstmate-codexapp` | before coordinating a visible Codex Desktop thread, evaluating a Codex App backend request, or reconciling Codex Desktop smoke evidence |
| `firstmate-coding-guidelines` | before changing firstmate's shared tracked material (section 1's list), directly or by brief |
