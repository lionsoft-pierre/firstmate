...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `no-mistakes`, this also covers an installed version older than 1.68.0, because this repo's PR gate requires structured pipeline attestation bound to the current PR head, and older builds either do not write it or leave it stale after their own CI-repair pushes.
