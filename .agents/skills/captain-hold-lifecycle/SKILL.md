...
Put the question in the hold reason and the answers the captain can pick on the hold itself, one `--option <value>=<label>` each plus `--recommend <value>` for your recommendation, so the model-free fleet board cards the call with those exact choices instead of a freeform box alone.
Keep one held task per genuine gate: a multi-question review is one held task pointing at its report, not a row per question, and because the board cards each held task exactly once, consolidate that task's questions and options on its one hold.
