...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${FM_FAKE_NO_MISTAKES_VERSION:-no-mistakes version v1.68.0 (fake) 2026-06-27T00:02:18Z}"
  exit 0
fi
exit 0
SH
...
minimum no-mistakes version is accepted^no-mistakes version v1.68.0 (fake)^empty
newer no-mistakes minor is accepted^no-mistakes version v1.69.0 (fake)^empty
newer no-mistakes major is accepted^no-mistakes version v2.0.0 (fake)^empty
older no-mistakes minor reports an upgrade^no-mistakes version v1.67.9 (fake)^missing
attestation-only build below the head-bound floor reports an upgrade^no-mistakes version v1.46.0 (fake)^missing
