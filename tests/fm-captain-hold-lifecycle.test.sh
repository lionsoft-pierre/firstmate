...
# `<origin>-decision-<key>` identities as plain task ids, short decision keys in
# recorded metadata, records the retired shim wrote, a concrete-origin binding,
# and the chat fallback for old rows.
test_legacy_identities_keep_working() {
  ...
  hold="$id-decision-pick-one"
  out=$(run_captain "$home" hold "$hold" --origin "$id" \
    --title "Pick one" --reason "captain choice pending" --repo sample) \
    || fail "the legacy-shaped hold path failed"
  [ "$out" = "$hold" ] || fail "the legacy-shaped hold did not print the composed identity: $out"
  run_captain "$home" hold "$id-decision-keep-two" --origin "$id" \
    --title "Keep two" --reason "captain second choice pending" --repo sample >/dev/null \
    || fail "the second legacy-shaped hold failed"
  show=$(tasks_in "$home" show "$hold" --full)
  assert_contains "$show" "hold_kind: captain" "the legacy-shaped row is not a plain captain-held task"
  ...
  # A legacy row answers like any other captain call.
  printf 'Use route north.\n' > "$home/route.txt"
  run_captain "$home" answer "$hold" --decision-file "$home/route.txt" >/dev/null \
    || fail "answering a legacy-shaped row failed"
  show=$(tasks_in "$home" show "$hold" --full)
  assert_contains "$show" "state: done" "the answer did not close the legacy-shaped row"
  assert_contains "$show" "Use route north." "the answer lost the captain decision"

  # A routed record the retired shim wrote is recognized wherever a record is
  # read: a closed row carrying one satisfies the completion gate unchanged.
  old_hold="$id-decision-old-route"
  run_captain "$home" hold "$old_hold" --origin "$id" \
    --title "Old routed choice" --reason "captain old route pending" --repo sample >/dev/null
  ...
  tasks_in "$home" "done" "$old_hold" >/dev/null

  # A decline is the same recorded answer.
  ...
  run_captain "$home" answer "$id-decision-keep-two" --decision-file "$home/decline.txt" >/dev/null \
    || fail "declining a legacy-shaped row failed"
  run_captain "$home" verify "$id" >/dev/null \
    || fail "closed legacy rows did not satisfy the completion gate"

  # A concrete-origin binding (a pre-collapse record) makes short channel keys
  # resolve through the composed identity.
  run_captain "$home" hold "$id-decision-third-choice" --origin "$id" \
    --title "Third choice" --reason "captain third choice pending" --repo sample >/dev/null
  run_captain "$home" bind legacy-src "$id" >/dev/null || fail "the concrete-origin bind path failed"
  ...
  run_captain "$home" hold "$id-decision-fourth-choice" --origin "$id" \
  ...
  pass "legacy identities, metadata, bindings, and the retired shim's records keep working"
}
...
# An answer keeps the option lines in the record, so a later question on the
# same task must not inherit the earlier question's choices: a new hold on a
# task that is no longer held drops them, while an active re-hold keeps them.
test_a_new_hold_drops_the_previous_options() {
  local home out show
  home=$(make_home hold-options-reset)
  tasks_in "$home" add ship-call "Ship the cutover" --kind ship --repo sample >/dev/null 2>&1 \
    || fail "could not seed the gated work item"
  run_captain "$home" hold ship-call --reason "captain window choice pending" \
    --option now="Cut over now" --option friday="Wait for Friday" --recommend friday >/dev/null \
    || fail "the first hold with options was refused"
  printf 'Friday it is.\n' > "$home/pick.txt"
  run_captain "$home" answer ship-call --decision-file "$home/pick.txt" --release >/dev/null \
    || fail "answer --release failed on the held work item"
  show=$(tasks_in "$home" show ship-call --full)
  assert_contains "$show" "Option: now = Cut over now" "the answer did not keep the option lines in the record"
  run_captain "$home" hold ship-call --reason "captain rollback call pending" >/dev/null \
    || fail "a new hold without options was refused"
  out=$(PATH="$home/fakebin:$PATH" FM_HOME="$home" FM_STATE_OVERRIDE="$home/state" \
    FM_DATA_OVERRIDE="$home/data" FM_CONFIG_OVERRIDE="$home/config" "$BEARINGS" --board) \
    || fail "the board projection failed: $out"
  printf '%s' "$out" | jq -e '
    .captains_call[] | select(.key == "ship-call")
      | .about == "captain rollback call pending" and (.options | length) == 0
        and (has("recommend_value") | not) and .allow_freeform == true and .close == "release"
  ' >/dev/null || fail "a new hold inherited the answered question's options: $out"
  run_captain "$home" hold ship-call --reason "captain rollback call pending" --option revert="Revert it" >/dev/null \
    || fail "an active re-hold with options was refused"
  run_captain "$home" hold ship-call --reason "captain rollback call pending" >/dev/null \
    || fail "an active re-hold without options was refused"
  out=$(PATH="$home/fakebin:$PATH" FM_HOME="$home" FM_STATE_OVERRIDE="$home/state" \
    FM_DATA_OVERRIDE="$home/data" FM_CONFIG_OVERRIDE="$home/config" "$BEARINGS" --board)
  printf '%s' "$out" | jq -e '.captains_call[] | select(.key == "ship-call") | [.options[].value] == ["revert"]' \
    >/dev/null || fail "an active re-hold without options dropped the current ones: $out"
