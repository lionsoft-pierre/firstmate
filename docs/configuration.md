...
The inventory below is that layout, file by file; `AGENTS.md` section 2 points here rather than restating it.
A `state/` entry marked never-touch is an internal record that only its owning script writes.
...
state/               runtime records and signals; gitignored
  ...
  <id>.meta          task metadata; each producer script's header owns its exact fields and mutation contract, with this document routing operator-facing backend and trace-context details
  ...
  .bearings-board-digest .bearings-board-refresh.log .bearings-board-refresh.lock .bearings-board-refresh-pending  live fleet-board refresh records: the last published payload digest whose mtime, touched on every attempt, paces the watcher's refresh, the bounded best-effort failure log, the single-flight lock, and the pending marker a refresh that found the lock held leaves so the lock holder, a refresh or a build, runs one follow-up before releasing; bin/fm-bearings-board.sh owns them (docs/configuration.md "Live fleet board")
...
The inventory above names each file and its owner; wake, watcher, away-mode, and Relay-specific state mechanics remain with those named scripts and reference sections rather than being restated here.
...
The watcher runs that refresh as a detached best-effort child on `FM_BOARD_REFRESH_INTERVAL` and on every coalesced status signal, exactly as it publishes `state/home-summary.json`; session start, spawn, teardown, `bin/fm-pr-check.sh`, and every `bin/fm-captain-hold.sh` hold, answer, and reconcile outcome call the same refresh with `--detach`, which hands the work to a detached best-effort child and returns at once, so the board follows the changes the watcher cannot see and no call site ever waits on a fleet snapshot.
...
The payload digest, `generated` stamp excluded, lives in `state/.bearings-board-digest`, whose mtime is the last-attempt stamp the cadence reads; every attempt touches it, a failed one included, so a failing generation is retried on `FM_BOARD_REFRESH_INTERVAL` and never on every poll, and best-effort failures append to the bounded `state/.bearings-board-refresh.log`.
`build` records the digest and the check stamp of the payload it published, so the watcher's next tick does not republish an identical board under the page the captain just opened; it generates, publishes, and records under the same single-flight lock, waiting for an in-flight refresh within `FM_BOARD_REFRESH_TIMEOUT` rather than yielding, so a refresh already deriving can neither republish identical content over the fresh build nor be overwritten by a payload the build derived before it ran, and a refresh that yields while the build holds the lock leaves the pending marker that the build honors with the same single follow-up before releasing.
A refresh that finds the single-flight lock held touches `state/.bearings-board-refresh-pending` beside the digest and yields; the lock holder checks that marker after its own publish, on the refreshed and the unchanged paths alike, and when present clears it and runs exactly one more generate-and-publish before releasing the lock, so a change that arrives mid-refresh reaches the board without waiting for the next cadence, while a marker touched during that follow-up waits for the next refresh.
The watcher touches the same marker when a cadence tick or a coalesced status signal finds its own refresh child still alive, so it never runs two children at once and the running child still folds that change into its single follow-up publish.
Every attempt that derives a payload, changed or not, also rewrites `$FM_HOME/.lavish/bearings-board-checked.js` with the epoch of that check; the open page loads that sibling through a script tag every 30 s and renders its footer as the last change, from the payload's `generated` stamp, and the last check, from the sibling, so a dead watcher reads as stale while a quiet fleet reads as current.
