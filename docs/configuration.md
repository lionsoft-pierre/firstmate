...
`build` records the digest and the check stamp of the payload it published, so the watcher's next tick does not republish an identical board under the page the captain just opened.
Refresh and build share one single-flight lock and one pending marker, `state/.bearings-board-refresh-pending`, and the complete coordination rule is written once in the REFRESH COORDINATION paragraph of `bin/fm-bearings-board.sh`'s header, which the watcher's signal and cadence paths follow.
...
The essential universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.68.0 or newer, compatible gh-axi, chrome-devtools-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
