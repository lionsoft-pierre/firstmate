...
5. Ordinary work, turn completion, session replacement (`/new`, `/resume`, `/fork`, reload), and ordinary signal, stale, check, heartbeat, or other wake handling: do not call `fm_watch_arm_pi` again because continuity is extension-owned rather than model-memory-owned.
6. Missing, failed, or unhealthy cycle only: if a later notification explicitly reports one of those repair conditions, drain queued wakes, inspect the failure text, call `fm_watch_arm_pi`, and restart the selected Pi-family executable with both extensions loaded if needed.
...
The watcher extension owns every successor launch, generation handoff, bounded retry, and watcher-failure alarm; `.pi/extensions/fm-primary-pi-watch.ts` and [`watcher-continuity.md`](../watcher-continuity.md) own that contract.

The supervision branch is default-on: [docs/pi-supervision-branch.md](../pi-supervision-branch.md) owns which rows it takes in each posture, how its outcomes are delivered and re-presented, and the away posture under `state/.afk-contract`.
This conversation still receives every fleet-wide or unresolvable wake, the branch's wakes when it is unavailable, and every watcher-failure alarm regardless of posture, so the arm and repair contract above is unchanged.
A captain-facing branch outcome arrives here as one hidden supervision processing request listing each `[seq N] task: summary` it covers; while away it waits for that request until the record is archived.
