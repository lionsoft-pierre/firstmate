...
- Recovery of an existing presentation journal cannot degrade to the flat layout without abandoning its exact restart binding, so it queues behind a live holder of the shared presentation lock instead of refusing on first contention, and refuses only when that wait is exhausted or nothing live holds the lock; default-on makes that refusal reachable in any Herdr home.
