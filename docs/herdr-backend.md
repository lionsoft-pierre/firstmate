...
- Recovery of an existing presentation journal cannot degrade to the flat layout without abandoning its exact restart binding, so it queues behind a live holder of the shared presentation lock instead of refusing on first contention, and refuses only when a single live holder keeps it past that wait (the wait restarts for each new holder) or nothing live holds the lock; default-on makes that refusal reachable in any Herdr home.
- The queue is what makes a concurrent cross-home resume work at all: a spawn that wins the lock keeps it from its projection or reclaim through worktree creation, metadata publication, harness launch, and the focus-safe send, which always outlasts the short wait that suits the brief API critical sections of cleanup and task kill.
