...
One concurrent cross-home recovery case refused under contention on a loaded machine and passed on an immediate rerun; recovery then refused on the first contention with a sibling spawn that held the presentation lock, and it now queues behind a live holder instead (the current contract is under "Operational compromises" in [the Herdr backend doc](../herdr-backend.md)).
