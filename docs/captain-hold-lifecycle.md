...
A keyed answer that replays a pre-collapse record with its historical digest is idempotent, so nothing already closed needs rewriting.
The one-release command-mapping shim `bin/fm-decision-hold.sh` has been removed; a brief that still names it uses the `bin/fm-captain-hold.sh` commands above, with a legacy row addressed by its full `<origin>-decision-<key>` task id.
